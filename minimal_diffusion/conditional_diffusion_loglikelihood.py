...
class UNet1DTrainer:
    def __init__(self, model, train_dataset, masks = None, val_dataset=None, inpaint = False, batch_size=32, lr=1e-4, weight_decay=1e-5, device=None, timestep_range=(0, 1000), save_path = "./Default"):
        ...
        # with torch.no_grad():
        #     dummy_input = torch.randn(1, self.in_channels, self.feature_size).to(self.device)
        #     dummy_timestep = torch.tensor([0], dtype=torch.long, device=self.device)
        #     dummy_label = torch.tensor([0], dtype=torch.long, device=self.device)
        #     _, dummy_latent = self.model(dummy_input, dummy_timestep, class_labels=dummy_label, return_latent=True)
    ...
    def train(self, epochs=10, log_interval=10):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in pbar:
                ...
                # Compute loss
                diff_loss = self.criterion(noise_pred, noise)
                # Classification loss
                #cls_logits = self.classifier(latent.mean(dim=-1))  # Pool across time if needed
                #cls_loss = self.classification_criterion(cls_logits, class_labels)

                # Combine
                alpha = 1.0  # Tune this weight
                total_loss = diff_loss #+  alpha * cls_loss
                total_loss.backward()
    ...
    def generate_samples_likelihood(self, num_samples=100, num_steps=20, data_shape=(1, 940), class_labels: Optional[torch.Tensor] = None, inpaint_masks = None):
        """
        ...
        with torch.no_grad():
            ...
            if self.inpaint == True:
                ...
                if self.in_channels == 1:
                    ...
                    mask = inpaint_masks.float().round() 
            ...
            total_loss = torch.zeros(image.size(0))
            for t in scheduler.timesteps:
                noise = torch.randn_like(image)
                noisy = scheduler.add_noise(image, noise, t)

                if binary_condition is not None:
                    input_tensor = torch.cat((noisy, binary_condition), dim=1)
                else:
                    input_tensor = noisy

                pred = self.model(input_tensor.to(device), torch.tensor([t] * num_samples, device=device), class_labels=class_labels).sample.cpu()

                if self.inpaint:
                    inpaint_region = (inpaint_masks == 1).float()
                    loss = F.mse_loss(pred * inpaint_region, noise * inpaint_region, reduction='none')
                else:
                    loss = F.mse_loss(pred, noise, reduction='none')

                total_loss += loss.mean(dim=[1, 2])

            likelihood_scores = -total_loss
            return image, likelihood_scores
